...
class WorkSheetsManager:
    ...
    def write(self, place: tuple[DataFrame | Any, str]) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Ghi dữ liệu vào Excel thất bại tại vùng {rng}") from e
    ...
    def get_data_range(self, rng: str, get_only_endpoint: bool = False) -> tuple[str, str] | str:
        ...
        try:
            ...
            start_p, end_p = point
            last_row: int = (
                self.sheet.range(f"A{self.sheet.cells.last_cell.row}")
                .end("up")
                .row
            )
...
class WorkBookManager:
    ...
    def get_sheet(self, sheet_name: str) -> WorkSheetsManager:
        try:
            ...
            raise RuntimeError(f"Excel đang bận hoặc tên sheet không đúng: {sheet_name}") from e
